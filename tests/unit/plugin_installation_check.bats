...
@test "session tracking prevents redundant directory existence checks" {
  # Create a mock plugin that exists
  local plugin_name="existing-plugin"
  local plugin_dir="${PULSE_DIR}/plugins/${plugin_name}"
  mkdir -p "${plugin_dir}/.git"
  echo "test" > "${plugin_dir}/${plugin_name}.plugin.zsh"
  ...
  run zsh -c "
    ...
    source '${PULSE_ROOT}/lib/plugin-engine.zsh'
    _pulse_init_engine
    _pulse_discover_plugins 2>&1 | grep -c 'Skipping installation check'
  ...
  [ "$status" -eq 0 ]
  # Second run should skip the check
  [[ "$output" =~ "1" ]] || [[ "$output" =~ "Skipping installation check" ]]
}
